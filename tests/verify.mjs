...
const privateDataPath = path.join(workspace, "private-source", "Escocia-datos-privados.json");
...
assert.ok(!publicFiles.some(file => /Escocia-datos-privados|\.mp3$|\.zip$|\.pdf$/i.test(file)), "Hay datos privados en la carpeta pública");

const totalBytes = trip.audioManifest.reduce((sum, track) => sum + track.size, 0);
